...
<head>
    ...
    <style>
        * {
        ...
        .cell.number-1 { color: #FF6B35; }
        .cell.number-2 { color: #58508D; }
        .cell.number-3 { color: #00A896; }
        .cell.number-4 { color: #F4A261; }
        .cell.number-5 { color: #2A9D8F; }
        .cell.number-6 { color: #E76F51; }
        .cell.number-7 { color: #264653; }
        .cell.number-8 { color: #E9C46A; }
...
<body>
    <div class="container">
        <div class="game-header">
            <div class="mines-counter">雷: <span id="remainingMines">十</span></div>
            <button class="smiley-btn" onclick="startNewGame()">🇨🇳</button>
            <div class="timer">时: <span id="gameTimer">〇</span></div>
        </div>
        ...
        <div class="game-controls">
            <div class="control-group">
                <label>宽度:</label><input type="number" id="width" value="9" min="5" max="15">
                <label>高度:</label><input type="number" id="height" value="9" min="5" max="15">
            </div>
            <div class="control-group">
                <label>地雷数:</label><input type="number" id="mines" value="10" min="1" max="30">
            </div>
    ...
    <script>
        function toChineseNumber(num) {
            const chineseNumbers = {0:'〇',1:'一',2:'二',3:'三',4:'四',5:'五',6:'六',7:'七',8:'八',9:'九'};
        ...
        const BOSS_CHANCES = {5:0.15,10:0.30,20:0.40,30:0.50,40:0.60,50:0.70};

        function getBossChance() {
            let maxChance = 0;
            for (const [wins, chance] of Object.entries(BOSS_CHANCES)) {
                if (totalWins >= parseInt(wins) && chance > maxChance) maxChance = chance;
            }
            return maxChance;
        }
        ...
        function updateBossDisplay() {
            document.querySelectorAll('.cell').forEach(cell => {
                ...
                if (bossCells.has(key)) {
                    cell.classList.add('boss-cell', 'boss-flashing');
                } else {
                    cell.classList.remove('boss-cell', 'boss-flashing');
                }
            });
        }
        ...
        function startBossMovement() {
            ...
            bossMoving = true;
            moveBoss();
            setInterval(() => bossActive && bossMoving && moveBoss(), 3000);
        }

        function moveBoss() {
            const dx = Math.floor(Math.random() * 61) - 30;
            const dy = Math.floor(Math.random() * 61) - 30;
            const newBossCells = new Set();
            bossCells.forEach(cellKey => {
                const [x, y] = cellKey.split(',').map(Number);
                let newX = Math.max(0, Math.min(29, x + dx));
                let newY = Math.max(0, Math.min(29, y + dy));
        ...
        function clearFlagsUnderBoss() {
            bossCells.forEach(cellKey => game.flaggedPositions.delete(cellKey));
            displayGame();
        }
        ...
        function handleBossClick(x, y) {
            ...
            bossCheckTimeout = setTimeout(checkBossVictory, 3500);
        ...
        function checkBossVictory() {
            ...
            for (let y = 0; y < 30; y++) {
                for (let x = 0; x < 30; x++) {
                    ...
                    if ((!cellState || cellState !== 'revealed') && !bossCells.has(key)) {
            ...
            allCellsAreMines ? bossVictory() : bossDefeat();
        }

        function bossVictory() {
            bossActive = bossMoving = false;
            const bossCellsElements = document.querySelectorAll('.boss-cell');
            let fallDistance = 0;
        ...
        function showGameOver(isVictory) {
            document.getElementById('gameOverText').textContent = isVictory ? '恭喜！你赢了！' : '游戏结束！你输了！';
        ...
        class MinesweeperGame {
            ...
            generateGame(seed = null) {
                ...
                this.stopTimer();
                this.startTime = null;
                const field = Array(this.height).fill().map(() => Array(this.width).fill(0));
                return {field: field, seed: this.seed, width: this.width, height: this.height};
            }
            
            placeMines(firstX, firstY, field) {
                const minesPositions = [];
                const totalCells = this.width * this.height;
                if (this.minesCount >= totalCells) this.minesCount = totalCells - 1;
                while (minesPositions.length < this.minesCount) {
                    ...
                    if ((x === firstX && y === firstY) || Math.abs(x - firstX) <= 1 && Math.abs(y - firstY) <= 1) continue;
                    const position = `${x},${y}`;
                ...
                minesPositions.forEach(pos => {
                    ...
                    for (let dx = -1; dx <= 1; dx++) {
                        for (let dy = -1; dy <= 1; dy++) {
                            ...
                            if (nx >= 0 && nx < this.width && ny >= 0 && ny < this.height && field[ny][nx] !== -1) field[ny][nx]++;
                        }
                    }
                });
            }
            ...
            revealCell(x, y, field) {
                ...
                if (!this.cellStates[`${x},${y}`]) {
                    ...
                    this.revealedCount++;
                    if (field[y][x] === 0) this.revealEmptyArea(x, y, field);
                }
            ...
            revealEmptyArea(x, y, field) {
                ...
                while (queue.length > 0) {
                    ...
                    const key = `${cx},${cy}`;
                    if (visited.has(key) || this.flaggedPositions.has(key)) continue;
                    visited.add(key);
            ...
            handleCellClick(x, y) {
                ...
                if (this.currentMode === 'reveal') {
                    if (!this.flaggedPositions.has(key)) this.revealCell(x, y, gameData.field);
                } else if (this.currentMode === 'flag') this.toggleFlag(x, y);
            }
            
            toggleFlag(x, y) {
                const key = `${x},${y}`;
                if (this.cellStates[key] === 'revealed') return;
                if (this.flaggedPositions.has(key)) this.flaggedPositions.delete(key);
                else this.flaggedPositions.add(key);
            }
            ...
            chordClick(x, y) {
                ...
                for (let dx = -1; dx <= 1; dx++) {
                    for (let dy = -1; dy <= 1; dy++) {
                        ...
                        if (nx >= 0 && nx < this.width && ny >= 0 && ny < this.height) {
                            ...
                            if (this.flaggedPositions.has(neighborKey)) flagCount++;
                            else if (!this.cellStates[neighborKey]) cellsToReveal.push([nx, ny]);
                        }
                    }
                }
                if (flagCount === cellValue) {
                    ...
                    for (const [nx, ny] of cellsToReveal) {
                        if (gameData.field[ny][nx] === -1) hitMine = true;
                        ...
                        for (let dy = -1; dy <= 1; dy++) {
                            ...
                            if (nx >= 0 && nx < this.width && ny >= 0 && ny < this.height) {
                                ...
                                if (!this.cellStates[neighborKey]) {
                                    if (this.flaggedPositions.has(neighborKey)) this.highlightedCells.add(neighborKey + '-flag');
                                    else this.highlightedCells.add(neighborKey);
                                }
                            }
                        }
                    }
            ...
            clearHighlight() { this.highlightedCells.clear(); }
            ...
            checkWinCondition(field) {
                ...
                if (this.revealedCount === totalSafeCells) {
                    ...
                    if (!bossActive) {
                        ...
                        if (checkBossSpawn()) {
                            setTimeout(() => {
                                if (confirm(`你赢了 ${totalWins} 次！BOSS 出现了！准备战斗吗？`)) activateBossMode();
                            }, 500);
                        }
                    }
                }
            ...
            getRemainingMines() { return this.minesCount - this.flaggedPositions.size; }
            startTimer() { this.startTime = Date.now(); this.timer = setInterval(updateTimer, 1000); }
            stopTimer() { if (this.timer) { clearInterval(this.timer); this.timer = null; } }
            getElapsedTime() { return this.startTime ? Math.floor((Date.now() - this.startTime) / 1000) : 0; }
        ...
        function generateWithSeed() {
            ...
            if (!seedInput) { alert('请输入种子号'); return; }
            const width = parseInt(document.getElementById('width').value) || 9;
            const height = parseInt(document.getElementById('height').value) || 9;
            const mines = parseInt(document.getElementById('mines').value) || 10;
            game.width = width;
            ...
            for (let y = 0; y < game.height; y++) {
                for (let x = 0; x < game.width; x++) {
                    ...
                    if (cellState === 'revealed') {
                        ...
                        if (game.flaggedPositions.has(key)) cell.classList.add('flag');
                    }
                    ...
                    if (cellState === 'revealed' && value > 0) {
                        cell.addEventListener('mousedown', (e) => {
                            if (e.button === 0) handleChordStart(x, y);
                        });
                        cell.addEventListener('mouseup', (e) => {
                            if (e.button === 0) handleChordEnd(x, y);
                        });
                    }
